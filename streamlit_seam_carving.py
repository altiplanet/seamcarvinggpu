...
def process_frame(frame, target_width, target_height):
    """
    Processes a video frame, resizing it using seam carving.
    """
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    img_array = np.array(frame)

    # Check dimensions of the frame
    if img_array.shape[0] > target_height:
        img_array = remove_horizontal_img(img_array, target_height)
    else:
        img_array = add_horizontal(img_array, target_height)

    if img_array.shape[1] > target_width:
        img_array = remove_vertical_img(img_array, target_width)
    else:
        img_array = add_vertical(img_array, target_width)
    ...
    if uploaded_file is not None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not cap.isOpened():
                ...
                desired_height = st.number_input("Desired Height", min_value=1, max_value=orig_height, value=orig_height)
                desired_width = st.number_input("Desired Width", min_value=1, max_value=orig_width, value=orig_width)
                num_workers = st.slider("Number of Workers", min_value=1, max_value=8, value=2)
                ...
                if st.button("Resize Video"):
                    ...
                    if not resized_path.endswith(".mp4"):
                        ...
                        h264_resized_path = convert_to_h264(resized_path, h264_resized_path)
                    ...
                    st.write(f"Output video dimensions: {desired_width}x{desired_height}")
